...
// import { queryFunctions } from '../ApiService'; // Adjust path as needed
...
//         if (!accessToken) {
//             throw new Error('You are already logged out. No session found.');
//         }
//         console.log('Token being sent to logout:', accessToken); // Debug
//         return queryFunctions.performLogout(accessToken);
//     },
//     onSuccess: () => {
...
//             : error.message; // Use the custom message from the throw
//         Swal.fire({
//             icon: 'error',
//             title: 'Logout Failed',
//             text: errorMessage,
...
const Sidebar = () => {
  ...
  const userType = localStorage.getItem('userType') || 'doctor'; // Default to doctor if not set
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      ...
      console.log('Token being sent to logout:', accessToken);
      return queryFunctions.performLogout(accessToken);
    },
    onSuccess: () => {
      localStorage.removeItem('accessToken');
  ...
  // Filter nav links based on user type
  const navLinks = userType === 'receptionist'
    ? allNavLinks.filter(link => 
        link.label === "Dashboard" || 
        link.label === "Appointment" || 
        link.label === "All Patient" || 
        link.label === "Logout")
    : allNavLinks;

  // Bottom nav links (exclude Logout for small screens)
  const bottomNavLinks = navLinks.filter(link => link.to !== "/doctor/logout");
      ...
      <aside>
        <nav
          className={`sidebar rounded-xl bg-background text-text shadow-xl border-r-2 border-primary h-full fixed left-0 top-0 transition-all duration-500 ease-in-out z-[90] 
            ...
            ${isCollapsed ? "w-12" : "w-64"} ${isSmallScreen ? "hidden" : "sm:block"}`}
        >
          {/* User Info Section with Profile */}
          <div className="flex items-center p-3 border-b border-gray-200 relative group">
            {isCollapsed ? (
              <Tooltip content="Dr. John Doe">
                <NavLink to="/doctor/profile">
